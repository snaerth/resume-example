		...
		return rows.map((row, i) => {
			return (
				<div className="processbar-row" key={i}>
					<div className={classnames('processbar-left', 'processbar-left-' + id)}>
						<h2>{row.name}</h2>
					</div>
					<div className={classnames('processbar-right', 'processbar-right-' + id)}>
	...
	render() {
		return (
			<div className="processbar-container">
				{this.renderRows(this.props.data, this.props.id)}
			</div>
		);
	}
}
